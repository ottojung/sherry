...
(cond-expand
 (guile
  (define-module (lib scheme-imports)
    :export (r7rs-main guile-main czempak-main file-read-first-expression do-initialize-stdlib-exports is-guile-decl?)
    ...
    :use-module ((euphrates hashmap) :select (alist->hashmap hashmap->alist hashmap-foreach hashmap-ref hashmap-set! make-hashmap))
...
(define (guile-main --import-everything <filepath>)
  ...
  (let* ((fullname (if (absolute-posix-path? <filepath>) <filepath> (append-posix-path (get-current-directory) <filepath>)))
         ...
         (files (list-deduplicate/reverse (map (comp (list-minimal-element-or #f string-length)) files/grouped)))
...
(define (do-initialize-stdlib-exports <root> <files...>*)
  ...
  (define files
    (list-deduplicate/reverse (map (comp (list-minimal-element-or #f string-length)) files/grouped)))
